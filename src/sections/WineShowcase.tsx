...
import { Wine, Sparkles, Thermometer, Clock, ArrowRight, ChevronLeft, ChevronRight, RefreshCw, CheckCircle, Github, GitBranch } from 'lucide-react';
import { wineShowcaseConfig } from '../config';

// Icon lookup map for dynamic icon resolution from config strings
const iconMap: Record<string, React.ComponentType<{ className?: string }>> = {
  Wine, Sparkles, Thermometer, Clock, RefreshCw, CheckCircle, Github, GitBranch
};
...
export function WineShowcase() {
  ...
  const globalFeatures = wineShowcaseConfig.features;
  const quote = wineShowcaseConfig.quote;
  const wine = wines[activeWine];
  const features = wine.features || globalFeatures;
      ...
      <div className="container-custom relative">
        ...
        <div className="grid lg:grid-cols-5 gap-8 lg:gap-12 items-center">
          ...
          <div className="slide-in-left lg:col-span-2 order-2 lg:order-1">
            ...
            <button
              onClick={() => {
                const target = wine.ctaLink || '#contact';
              ...
              aria-label={wine.ctaText || wineShowcaseConfig.mainTitle}
            >
              {wine.ctaText || wineShowcaseConfig.mainTitle}
